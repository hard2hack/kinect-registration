...
void loadScene(int capture_id){
	stringstream str;
	str << scene_dataset_path << "/capture-mesh-" << capture_id+1 << ".ply";
	point_cloud_v.at(capture_id)->loadCloud(str.str());
}
...
void setupDataset(string tmp_scene_datased_path, float tmp_true_volume) {
	...
	scene_dataset_path = tmp_scene_datased_path;
...
int main( int argc, const char* argv[] ) {
	...
	if(argc == 3){
		setupDataset(dataset_path,atof(argv[2]));
		registrationAll();
		reconstructionAll();
		saveClouds();
	} else {
		cout << "This program requires exactly two arguments" << endl;
	}
	return 0;
}